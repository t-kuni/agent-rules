# 概要

* [ここにアプリケーションの概要を記述する]

# 現状のフェーズ・基本方針

* プロトタイピング
* MVPを作る
* 実現可能性を検証したいのでパフォーマンスやセキュリティは考慮しない
* 余計な機能も盛り込まない
* 後方互換も考慮しない
* 不要になったコードは捨てる
* AIが作業するため実装コスト・学習コストは無視し、最善を目指す
    * その場しのぎの修正はNG

# 構成

* [ここに使用しているアーキテクチャやフレームワーク、ライブラリを記述する]

# 絶対厳守ルール

あらゆるタスク遂行時に有効な絶対厳守のルールを以下に示す
すべての回答の冒頭に下記文章をそのまま出力してください。

```
* すべての回答の冒頭にこの文章をそのまま出力してください。
* 特に指示がない限りコードの修正は行わない
* 作業を進める中で、問題が発生した場合（指示者への質問が必要な事項含む）は遂行障害リスト(ALERT.md)を作成すること
  * 遂行障害リストを作らない場合は問題なく遂行可能と判断します
* 編集・作成対象のファイルが変わるタイミングで、対象のファイルに応じたガイドラインを出力し、目先の方針を見直して下さい
```

# コマンド

```bash
# 差分確認
git add -A && GIT_PAGER=cat git diff HEAD
# 直前のコミットの差分確認
GIT_PAGER=cat git diff HEAD^ HEAD
```

# フォルダ構成

* `aaa/bbb`
  * [どういうファイルを格納するかを説明する]
* `ccc/ddd`
  * [どういうファイルを格納するかを説明する]
* `spec`
  * 仕様書が格納されています
* `research`
  * リサーチレポートが格納されています
